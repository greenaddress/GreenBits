...
public class ListBitBoatTxsAdapter extends ArrayAdapter<BitBoatTransaction> {
  ...
  public ListBitBoatTxsAdapter(final Context context, final int resource,
      final List<BitBoatTransaction> objects, final String btcUnit) {
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    if (convertView == null) {
      ...
      returnedView = inflater.inflate(R.layout.list_element_bitboat, parent,
          false);
      holder = new Holder();
      holder.current = current;
      holder.textFirstBits = (TextView) returnedView.findViewById(R.id
          .firstBits);
      holder.textValueBtc = (TextView) returnedView.findViewById(R.id
          .listValueBtc);
      holder.textValueFiat = (TextView) returnedView.findViewById(R.id
          .listValueFiat);
      holder.textTimeLeft = (TextView) returnedView.findViewById(R.id
          .listTimeLeft);
      holder.textNumber = (TextView) returnedView.findViewById(R.id
          .listNumberText);
      ...
      holder.bitcoinScale = (TextView) returnedView.findViewById(R.id
          .listBitcoinScaleText);
      holder.bitcoinIcon = (TextView) returnedView.findViewById(R.id
          .listBitcoinIcon);
    ...
    final MonetaryFormat bitcoinFormat = CurrencyMapper.mapBtcUnitToFormat
        (btcUnit);
    holder.bitcoinScale.setText(Html.fromHtml(CurrencyMapper
        .mapBtcUnitToPrefix(btcUnit)));
    ...
    holder.textValueBtc.setText(bitcoinFormat.noCode().format(current
        .valueBtc));
    ...
    int left = Math.max(0, (int) ((3600000 - ((new Date()).getTime() -
        current.date.getTime())) / 60000));
    holder.textTimeLeft.setText(new Formatter().format(getContext()
        .getResources().getString(R.string.minutesLeft), left).toString());
    holder.textNumber.setText(
        (current.method == BitBoatTransaction.PAYMETHOD_POSTEPAY ? "Postepay" :
            current.method == BitBoatTransaction.PAYMETHOD_SUPERFLASH ?
                "Superflash" :
                current.method == BitBoatTransaction.PAYMETHOD_MANDATCOMPTE ?
                    "Mandat Compte" :
                    "") + " " +
            current.number);
